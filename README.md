...
- 预设功能集中到配置页：保留节点侧栏、内核栏、文件切换及源码编辑，在“限制”旁通过“入站操作”增加、修改或删除入站；高级字段、版本历史、差异和客户端入口也在此页。
...
- 入站表单与高级字段保存、任务创建使用同一事务，任务锁定本次配置版本；自动安装并入同一持久化任务，不依赖页面保持打开。源码编辑仍保留单独的保存与提交任务流程。未保存源码或节点快照与已保存版本不同会阻止入站操作，版本冲突保留弹窗草稿。节点与内核可通过旧链接或配置页链接恢复，草稿凭据不写入浏览器存储或 URL。
