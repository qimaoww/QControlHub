...
- Agent 通过既有 WSS 心跳上报 CPU、内存、根磁盘、默认路由接口地址、实时上下行速率及累计流量，并以独立的每秒指标推送刷新实时快照；节点页每 2 秒从带会话鉴权的同源接口刷新最新数值。
...
- 四个托管内核的运行日志由 QAgent 通过 WSS 实时汇集到面板；节点只使用有界内存和易失 journal，日志在面板 PostgreSQL 中保留 7 天并支持筛选查看。
...
- 管理员可为每个节点保留多条独立的添加节点命令；命令绑定节点名称、可重复安装且互不失效，删除某条添加记录只会停用对应命令。后续 WSS 握手使用 Ed25519 签名、时间窗和持久化 nonce 防重放。
