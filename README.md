...
- 面板首次安装可选择 Mihomo、Xray、sing-box、Shadowsocks Rust 默认能力；系统设置可修改新 Agent 的全局默认值，节点设置可独立开启或关闭，包括全局默认关闭的内核。关闭会停止服务，重新开启会恢复管理能力并启动已安装内核，启停成功后更新开关；未安装内核仅切换能力，不卸载内核、不删除配置，重装保留节点选择。
