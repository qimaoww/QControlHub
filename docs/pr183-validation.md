...
### 节点所有者对管理员隐藏（schema 58）

- 新增 `agents.admin_hidden` 与 `enrollment_tokens.admin_hidden`。所有者可在“添加节点”时勾选，或之后在节点设置里切换“不让管理员查看管理”。
- 开启后，管理员和兼容令牌在节点列表、详情、配置、任务、部署、日志、指标、流量、审计和安装凭据中一律看不到该节点（直接访问返回 `404`），改名、部署、启停、共享、安装命令和修改隐藏状态等管理动作同样被拒绝。
- 管理员和兼容令牌仍可通过节点设置里的“其他用户节点”目录只读查看名称、归属账号、在线状态、内核与端口，并保留删除该节点的能力；配置正文、日志、指标、任务与流量不在目录中。
- 所有者与显式共享接收者的访问不受影响；后台采集、计费、通知、Agent 连接和保留策略继续按所有者设置执行，所有者随时可恢复可见。
- 回归覆盖 `TestPR183AuditOwnerHiddenNodeVisibility`：管理员列表/详情/子资源全部 `404`、目录包含隐藏节点且非管理员收到 `403`、所有者切换后管理员恢复可见、管理员仍可删除隐藏节点。

### 实机功能验证（合并前）

在本机真实控制面（`bin/qcontrol-plane` + 独立 PostgreSQL 数据库）与真实 Agent 容器上执行，未使用 mock：

- 通过真实 HTTP API 完成 36 项断言：隐藏节点的凭据创建、真实注册、管理员列表/详情/配置/任务/部署/流量/概览/审计/指标/主机日志全部 `404` 或被过滤、只读目录与非管理员 `403`、所有者切换可见性、管理员无法替他人切换、管理员仍可删除隐藏节点、所有者审计保留；模板 `{{lan_ip}}` 在无 `metrics.read` 时返回 `403`、授权后返回 `200`；自动区域检测在无 `metrics.read` 时不返回 IP 或归属地。
- 真实 `qagent` 容器（OpenRC 模式）用隐藏凭据注册并建立 WSS 会话，上报真实网卡指标；管理员在列表与指标接口仍看不到该在线节点，目录显示其在线与隐藏标记；所有者保存配置后舰队 reconcile 仍覆盖隐藏节点（`GET /traffic-endpoints` 返回该端口）；共享给另一账号后，接收者只收到全局可路由地址，私网接口地址不下发。
- 真实 Chromium（headless，真实后端 + 同源代理）12 项 UI 断言全部通过：所有者看到“添加节点”弹窗中的“不让管理员查看管理此节点”复选框、节点设置里的“管理员可见性”开关可切换并恢复、具名管理员看到“其他用户节点”按钮、隐藏节点从常规管理员页面消失、目录弹窗列出该节点并显示“已对管理员隐藏”。
- 实测中发现并修复：管理员审计列表的 `detail` 仍带隐藏节点 ID（例如 `task.created` 的 “status mihomo agt_…”）。审计过滤现在同时排除 target 直接指向、任务/配置关联、`detail` 文本包含该 ID 以及隐藏凭据自身的条目；所有者仍保留完整审计。

### 合入后的修复（独立 PR）

合并后实测暴露两处回归，与账号删除入口一起在独立 PR 中修复，schema 版本保持 58、无新迁移：

- 节点顺序迁移：升级后拖拽顺序丢失。原有顺序保存在浏览器全局键 `qcontrolhub:node-card-order`，账号分区后改为按账号标识的新键，旧键不再被读取。现在首次读取时把旧列表迁移到当前账号的新键，只保留当前账号可见的节点 ID（不会把其他账号的顺序写进本账号），且已存在的新键永不被覆盖；回归见 `frontend/module_smoke.mjs` 的迁移断言。
- 目录范围：“其他用户节点”曾把管理员自己管理的升级前遗留节点（`owner_id=''`）列为“未知账号”。目录现在只返回 `owner_id` 非空的节点，管理员自己的旧节点继续在常规列表中管理；文案同步改为“仅列出归属其他账号的节点”。
- 账号删除：新增 `POST /api/v1/users/{id}/purge`（仅管理员，`204`）。界面在账号详情提供“删除账号”按钮，需连续两次确认；管理员账号与当前会话不提供该按钮。删除只移除账号本身：节点、配置、模板、安装凭据、任务与内核归属转入管理员范围，共享、配额关联和个人设置随账号删除，审计保留。回归见 `internal/store/users_purge_test.go` 与 `internal/api/users_integration_test.go` 的 `TestPurgeUserEndpointWithPostgreSQL`（管理员账号 `409`、重复删除 `404`、普通账号自删 `403`）。
- 实机验证（本机真实控制面 `bin/qcontrol-plane` + 独立 PostgreSQL 数据库 + 通过真实 `/agent/v1/enroll` 注册的节点 + 真实 Chromium 与同源代理）：接口 22 项断言全部通过（管理员范围的遗留节点不进入目录、其他账号节点带 `owner_username` 列出、非管理员目录 `403`、普通账号自删 `403`、管理员账号与当前会话 `409`、未知账号 `404`、删除后账号无法登录、节点与配置保留并转入管理员范围、目录移除、`user.purged` 审计、停用流程回归）；界面 16 项断言全部通过（浏览器全局顺序迁移到账号分区键并过滤不可见 ID、已有分区顺序不被旧键覆盖、目录只列其他账号节点且不列管理员范围节点、删除按钮仅对普通账号出现、取消确认不生效、连续两次确认后账号从列表与接口消失、管理员账号无删除按钮）。`make frontend-check` 与完整 `make test` 通过。
