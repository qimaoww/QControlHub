...
删除 Agent 是不可逆的身份吊销，不要求节点在线或响应：控制面在短事务中吊销身份、删除该节点的全部添加凭证并持久化清理任务，提交后主动取消当前认证 WSS，返回 `204`。节点及其配置、流量和订阅选择立即从相应查询中移除；未完成任务的失败标记，以及配置正文、修订、流量历史和日志的物理清理由独立后台任务完成。后台每 10 秒检查队列，清理失败延迟一分钟重试，服务重启后继续处理，不会撤销已成功的删除。相同 Ed25519 身份的后续握手返回 `401`。Agent 收到 `401` 后按指数退避重连而不退出进程，因此签名时间偏差等原因消除或身份重新注册后会自动恢复；如需重新注册，必须创建新的添加命令。
