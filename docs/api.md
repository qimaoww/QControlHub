...
删除 Agent 是不可逆的身份吊销：控制面先删除该节点的配置与修订、删除该节点的全部添加凭证、将其未完成任务标记为失败，再主动关闭当前认证 WSS；相同 Ed25519 身份的后续握手返回 `401`。Agent 收到 `401` 后按指数退避重连而不退出进程，因此签名时间偏差等原因消除或身份重新注册后会自动恢复；如需重新注册，必须创建新的添加命令。
