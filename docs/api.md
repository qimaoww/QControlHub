...
| `GET` | `/api/v1/ip-quality/{task_id}/archives/{4或6}` | 从数据库读取 SVG 图片，`?download=1` 下载；与报告历史相同的 agents.read、所有权及可见性校验 |
...
依赖、数据解释、上游许可、隐私及 schema 63 回滚要求见
...
删除 Agent 是不可逆的身份吊销，不要求节点在线或响应：控制面在短事务中吊销身份、删除该节点的全部添加凭证并持久化清理任务，提交后主动取消当前认证 WSS，返回 `204`。节点及其配置、流量和订阅选择立即从相应查询中移除；未完成任务的失败标记，以及配置正文、修订、流量历史和日志的物理清理由独立后台任务完成。后台每 10 秒检查队列，每批最多处理 256 行并独立提交；清理失败延迟一分钟重试，服务重启后继续处理，保留已完成的清理进度，不会撤销已成功的删除。相同 Ed25519 身份的后续握手返回 `401`。Agent 收到 `401` 后按指数退避重连而不退出进程，因此签名时间偏差等原因消除或身份重新注册后会自动恢复；如需重新注册，必须创建新的添加命令。
...
`GET /api/v1/agent-directory` 是管理员查看其他账号节点的只读目录，仅限管理员角色或部署时配置的兼容令牌；普通账号即使持有 `agents.read` 也返回 `403`。目录只包含 `owner_id` 非空且不属于当前登录账号的节点；升级前遗留、现由管理员范围（`owner_id=''`）持有的节点，以及具名管理员自己拥有的节点，继续出现在常规节点列表中，不会被列成其他账号。它返回 `[{"id":"agt_…","name":"edge-01","owner_username":"alice","status":"online","capabilities":["mihomo"],"ports":[21001],"admin_hidden":true}]` 形式的只读摘要，不含配置正文、日志、指标、任务或流量。管理员可据此调用 `DELETE /api/v1/agents/{id}` 删除任意账号的节点（含隐藏节点），删除沿用既有身份吊销与级联清理语义。

删除账号不删除节点或覆盖已有管理员配置。同一节点、同一内核已有管理员范围配置时，移交的冲突配置转为独立档案，保留配置 ID、版本与部署关联。移交节点的管理员隐藏标记清除；旧安装命令立即失效并清除可恢复凭据，重新安装须由管理员生成新命令。排队任务取消，执行中任务标记失败并废止旧结果租约，执行结果不明的内核状态仍保留；已运行服务和 Agent 身份不变，过期任务回执仍按既有协议拒绝。受影响的在线 Agent 收到流量策略刷新，其他账号的旧共享编辑修订失效，审计以已删除账号的 ID 和用户名保留追溯信息。
...
配置页的自动读取会在上述两种读取请求中携带 `"prefer_cached": true`。控制面若找到同一提交人、节点、内核和读取类型在最近 600 秒内成功校验的快照，以 `200` 返回该任务并标记 `"reused": true`，无需再次等待节点；没有命中时以 `201` 创建任务，或以 `200` 复用仍在执行或排队的同类读取任务。管理员也不跨账号复用缓存。此字段用于其他 action 时返回 `400`。缓存命中仍须通过完整的参数、当前权限、节点能力和服务映射校验，并与任务创建、下发共用节点事务锁。

存在排队或执行中的部署、导入、安装/切换内核或自动安装任务时，不复用成功快照。这些任务下发前即清除该节点与内核的两种读取快照；即使变更失败或执行结果丢失，也不会恢复旧缓存。下发前取消任务和普通的只校验任务不会失效未变化的快照。缓存密文不可解密时回退为读取任务，数据库或权限错误仍然返回失败。缓存正文继续加密存于 PostgreSQL，并在取快照时重新检查当前主机读取权限，不写入浏览器持久化存储。

页面内存中的节点快照也按相同的 600 秒窗口过期。用户手动刷新和托管配置部署前核验不携带缓存字段，创建或复用排在已排队配置变更之后的读取任务。部署前将本次强制读取结果与页面最初读取的 Agent 配置基线逐字比较，不一致则在保存配置和创建部署任务前停止。快照在读取过程中被其他任务失效时，页面至多重试一次强制读取；离开页面、切换配置来源或账号后不再重试。这是前端的提前差异检查，不是 Agent 执行时的原子比较并交换，无法阻止核验之后发生的外部修改或并发部署。
