...
配置页的自动读取会在上述两种读取请求中携带 `"prefer_cached": true`。控制面若找到同一权限范围、节点、内核和读取类型在最近 600 秒内成功校验的快照，直接以 `200` 返回该任务并标记 `"reused": true`，无需再次等待节点；没有命中时仍以 `201` 创建任务。此字段用于其他 action 时返回 `400`。页面内存中的节点快照也按相同的 600 秒窗口过期。用户手动刷新和部署前核验不携带该字段，始终读取节点；部署前会将本次强制读取结果与页面最初读取的 Agent 配置基线逐字比较，不一致则在保存配置和创建部署任务前停止。成功部署、导入或安装/切换内核也会立即清除该节点与内核的可复用快照。缓存正文继续加密存于 PostgreSQL，并在取快照时重新检查当前主机读取权限；不写入浏览器持久化存储。
