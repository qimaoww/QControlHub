...
“入站标签”支持自定义 1–64 个 Unicode 字符的名称，不能包含控制字符或首尾空白；保存时写入 SS Rust 的 `id`，重名会被拒绝。旧单端口配置以及没有 `id` 的多端口配置在预设编辑时固定原有标识，删除前一个端口不会让其他端口编号变化。这里修改的是服务端配置；客户端页的“修改显示参数”则是独立的单端口分享名称、连接地址与地址协议栈，保存后即时生效，不需要重新部署。
