...
按上游终端输出的版式（列宽、取自终端的配色、各库评分阈值）生成 SVG，存入面板
数据库并以图片内嵌。`ipapi`、`DB-IP` 的等级文案来自其厂商 API，JSON 里只有分数，
因此 Agent 保留上游打印的报告文本并把它转发给面板；面板优先使用该文案，缺失时
再按阈值推导。检测不再依赖上游上传端点的可用性，有效 JSON 报告一定会有对应存档图。
