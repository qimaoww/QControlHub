...
上游上传限时 20 秒，返回链接最多 2 KiB。上游在无法访问 `ipinfo.check.place`
时会降级为轻量报告并默认跳过上传；这是数据源不可达而非用户选择，因此集成仍会为
结构有效的轻量报告上传并导出链接（隐私模式除外）。上传接口返回 HTTP 错误
（例如 `upload.check.place` 被 Cloudflare 拦截返回 403）时不会写入空链接或错误页，
任务以“上游上传未返回报告链接”失败并提示检查节点出站网络；拿到链接后仍按原样校验。
