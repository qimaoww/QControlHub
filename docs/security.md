...
这是面板配置数据隔离，不是主机、进程或网络租户隔离。每台主机每种内核只有一个活动配置；校验不会替换运行配置，部署会替换同内核当前配置并重启服务。`tasks.execute`、`agents.manage`、`settings.manage` 等主机/全局权限应只授予可信操作者，日志、节点指标、端口流量和全局设置仍按原能力权限共享。Sub-Store 使用共用后端地址，修改地址仍需 `settings.manage`；远端组名必须全局唯一。

共享主机的 `read-config`、`read-managed-config`、`import-existing` 及配置快照读取仅限管理员，避免读取其他用户正在运行的完整配置。普通用户的手动配置页只编辑个人工作区，具备 `agent-config.write` 即可保存，无需执行任务权限；部署及校验另需 `tasks.execute`。模板应用同时需要 `templates.write` 和 `agent-config.write`。
