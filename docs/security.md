...
schema 55 中普通账号始终独立，可按权限创建安装凭据并管理自己的 Agent、设置、日志及集成。借用节点必须有明确共享分配，不能获得宿主服务、重装凭据、BBR、主机日志、Komari 或再次分享的权限；流量列表只包含自己的节点与获分配端口。授权每次从数据库核验，撤销对已有登录会话也有效。修改密码、权限、角色或停用账号会增加持久鉴权修订，使其他控制面进程中的旧会话也失效。用户管理仅限真正的管理员，单独授予 `users.manage` 不能越权。
...
升级至 schema 55 后，已有普通用户也进入独立范围，旧节点和安装凭据留在管理员范围，需要重新显式分配。运行策略、日志与指标保留由节点所有者的设置决定；配置修订、任务和审计保留由记录所有者决定。任务失败的私有诊断只发往任务所有者的 Webhook。浏览器敏感偏好按账号标识分区，旧账号的异步响应不得进入新账号页面。
