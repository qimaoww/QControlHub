...
该可见性不授予其他账号的报告读取权，也不向共享接收者开放计划。

Agent 运行上游官方一行命令
[xykt/IPQuality](https://github.com/xykt/IPQuality)（`bash <(curl -Ls https://IP.Check.Place)`），
不固定修订、不校验 SHA-256，以隐私模式运行（不向任何上游端点上传报告），
不接受面板提供的 shell、脚本 URL、路径或代理参数。缺少探针依赖时由上游一行命令
用节点包管理器非交互安装。报告有结构、大小、地址族和文件类型校验，
运行超时终止进程组；子进程不继承敏感环境。
这些措施不是不可信程序沙箱：上游脚本继承 Agent 当前身份及平台隔离，
本功能不增加权限，也不弱化现有 systemd 限制，OpenRC 的主机边界仍较弱。

手动检测/开启计划前会提示；探测会访问第三方 IP、媒体、邮件、DNS 及统计服务。
面板不发起任何出站请求，只把 Agent 回传的报告原文（有界、转义、仅解释 ANSI
颜色码）渲染成有界 SVG；渲染前后重检任务权限和租约。
JSON、SVG 原文与校验元数据只持久保存于面板数据库，Agent 的报告重传缓存仅在内存。
...
schema 58 中普通账号始终独立，可按权限创建安装凭据并管理自己的 Agent、设置、日志及集成。借用节点必须有启用且接收者已接受的明确内核分配；待接受、已拒绝或未分配内核不授予节点、配置、流量和任务访问权。所有者、管理员或其他用户不能代替接收账号确认；响应必须带邀请修订，旧修订不能接受修改后的分配。拒绝后须显式重新邀请，增加内核或撤销后恢复也需重新接受；仅缩减内核保留接受状态并撤销对应内核权限。借用者不能获得宿主服务、重装凭据、BBR、主机日志、Komari 或再次分享的权限；配置、历史、任务、客户端、Sub-Store 来源和流量按内核范围过滤。授权每次从数据库核验，退出或撤销对已有登录会话也有效，旧排队任务和已派发租约不会因重新接受而复活。修改密码、权限、角色或停用账号会增加持久鉴权修订，使其他控制面进程中的旧会话也失效；撤销执行权限或停用账号会同事务作废旧任务，重新授权不会恢复旧租约。用户管理仅限真正的管理员，单独授予 `users.manage` 不能越权。

节点所有者可以在添加节点时勾选、或之后在节点设置里切换“不让管理员查看管理”。开启后该节点对管理员和兼容令牌在节点列表、配置、任务、日志、指标、流量、审计和安装凭据中一律不可见，直接访问返回 404；管理员和兼容令牌仍可在“其他用户节点”目录中只读看到名称、归属账号、在线状态、内核与端口，并保留删除该节点的能力，但部署、改配置、启停、共享和修改隐藏状态仍被拒绝。目录只列出 `owner_id` 非空且不同于当前账号的节点；升级前遗留、由管理员范围持有的节点和具名管理员自有节点仍只在常规节点列表中出现。后台采集、计费、通知和 Agent 连接不受影响，所有者随时可恢复可见。

管理员可以永久删除普通账号（`POST /api/v1/users/{id}/purge`）。节点、配置、模板、任务历史和内核归属转入管理员范围，移交节点的隐藏标记随之清除；已有管理员工作区不被覆盖，冲突配置转为独立档案并保留版本与部署关联。旧安装凭据立即吊销并清除可恢复密文，避免原账号用旧命令重装接管后的节点；重新安装需要管理员生成新命令。排队任务取消，执行中任务失败且旧结果租约失效，已执行操作的不确定状态不会被误标为安全完成。共享邀请、共享接收者、端口配额关联、个人面板设置、Sub-Store 凭据与同步组随账号删除，端口流量历史与审计保留；审计同时保留账号 ID 和用户名。

删除事务按用户、安装凭据、节点、共享、任务/策略顺序加锁，与安装、任务派发和流量上报相协调；已在进行的注册先完成，再把新节点纳入移交。账号删除后的在途请求不能退化为兼容令牌权限。其他控制面实例的旧登录会话也会失效，存活账号和节点的共享修订递增，防止旧草稿恢复已删除的分配。管理员账号不能被删除，当前会话不能删除自己，均返回 `409`；删除期间新增共享接收者时返回 `409`，刷新后重试。删除不会主动断开 Agent 或终止已运行服务，受影响节点会收到流量策略刷新并使用管理员范围的保留策略；过期任务回执仍按既有协议拒绝。

以上安全移交用于后续账号删除。若已在旧版本中删除过账号，应检查并更换相关节点的旧安装命令；不能仅凭 `owner_id=''` 自动区分这类遗留凭据和合法的管理员凭据。
...
配置校验与部署在提交、领取/恢复及 Agent 执行时强制独立出口检查；无法为每个代理入站建立独立出站归属时拒绝执行，不再仅警告后原样部署。重复/歧义 JSON 字段、隐藏入口、动态端口、未知路由转交和可改写路由的管理 API 不能绕过检查。此处独立出口指独立出站标签或 socket mark，不保证独立公网 IP。精确迁移已有服务的所有者专用 `import-existing` 不代替该检查，也不代表存量配置已经转换；不适合的旧配置需修改后重新校验、部署。

这是面板权限与固定监听端口计费隔离，不是独立进程、文件系统或 OS 沙箱。内核进程仍共用主机运行身份与资源；不能作为运行恶意配置、宿主机本地用户或 root 的安全边界。旧版兼容令牌保留升级前的全局节点访问能力，不属于按账号隔离的身份：operator 令牌等同于管理员级运维凭据，可跨账号停止、部署或重装内核并重置他人的端口计费；auditor 与 readonly 令牌可跨账号读取主机日志和指标。它们应按管理员凭据保护、轮换与审计，并尽快迁移到具名账号；确需保留时不得把令牌交给需要相互隔离的账号。每个账号独立配置 Sub-Store 后端；相同后端内远端组名唯一，不同后端可重名且互不阻塞。把同一 Sub-Store 凭据交给多个账号也意味着它们可以绕过面板直接访问该外部服务；需要严格外部数据隔离时必须使用独立实例/凭据。完整操作见 [用户与 Agent 分配](agent-sharing.md)。

借用者不能执行 `read-config`、`read-managed-config`、`import-existing` 或读取宿主配置快照。自有节点在没有他人配置或状态不明的部署时可显式读取/导入；读取任务在执行和完成时再次核验当前部署归属，节点所有者也不能借排队任务取得共享者的完整配置。普通用户的手动配置页默认编辑个人工作区，具备 `agent-config.write` 即可保存，无需执行任务权限；部署及校验另需 `tasks.execute`。模板应用同时需要 `templates.write` 和 `agent-config.write`。工作区内附带的 Agent 指标同样要求 `metrics.read`，自动区域检测（含推导出的公网 IP 与归属地）也受该能力保护；手动标注的区域码不受影响。借用节点只返回可用于建立客户端连接的可路由地址，内网接口地址不会随共享暴露。

总览中的“面板主机”属于全局基础设施数据，使用独立的 `panel-metrics.read`，不随
`overview.read`、节点 `metrics.read` 或兼容令牌自动开放。管理员默认可读，普通用户
默认不授予；管理员可在用户权限中显式勾选。接口只返回控制面可见的资源计数，不返回
网卡地址、公网 IP、内核调优配置或原始采集错误，不增加容器特权或宿主机挂载。

升级至 schema 57 后，已有普通用户也进入独立范围，旧节点和安装凭据留在管理员范围，需要显式分配内核、邀请并接受。旧共享清空内核范围并转为待接受（已拒绝保持拒绝），累计账本、共享 ID 和端口保留，旧敏感任务租约及主机读取快照作废；升级须统一更新所有控制面实例及 Agent，不能混跑不校验邀请和内核范围的旧版。WSS 当前连接心跳确认支持内核分配及独立出口之前，共享策略保持阻断，不信任数据库缓存的旧特性。运行策略、日志与指标保留由节点所有者的设置决定；配置修订、任务和审计保留由记录所有者决定。任务失败的私有诊断只发往任务所有者的 Webhook。浏览器敏感偏好按账号标识分区，旧账号的异步响应不得进入新账号页面。
