...
节点所有者可以在添加节点时勾选、或之后在节点设置里切换“不让管理员查看管理”。开启后该节点对管理员和兼容令牌在节点列表、配置、任务、日志、指标、流量、审计和安装凭据中一律不可见，直接访问返回 404；管理员和兼容令牌仍可在“其他用户节点”目录中只读看到名称、归属账号、在线状态、内核与端口，并保留删除该节点的能力，但部署、改配置、启停、共享和修改隐藏状态仍被拒绝。目录只列出 `owner_id` 非空的节点，即真正归属其他账号的节点；升级前遗留、由管理员范围持有的节点仍只在常规节点列表中出现。后台采集、计费、通知和 Agent 连接不受影响，所有者随时可恢复可见。

管理员可以永久删除普通账号（`POST /api/v1/users/{id}/purge`），但只删除账号本身：该账号拥有的节点、配置、模板、安装凭据、任务历史和内核归属整体转入管理员范围，共享邀请、共享接收者、端口配额关联、个人面板设置、Sub-Store 凭据与同步组随账号一并删除，审计记录保留以备追溯。管理员账号不能被删除，发起删除的当前会话也不能删除自己，两种请求分别返回 `409`。删除不会终止 Agent 连接或中断采集，接管后的节点继续按管理员范围的保留策略运行。
