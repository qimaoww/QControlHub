...
schema 53 增加可选的 Agent 隔离与累计共享额度。旧用户保留原有访问范围；新建普通用户在页面上默认隔离。隔离用户只能访问启用的分配，不能管理主机、接入凭据、全局设置或读取主机日志；流量列表只包含自己的共享端口。授权每次从数据库核验，撤销对已有登录会话也有效。用户管理仅限真正的管理员，单独授予 `users.manage` 不能越权。

每台主机每种内核仍只有一个活动配置。涉及隔离用户时，不能覆盖另一用户运行中、执行中或结果不明的部署；先由管理员确认停机，再重新部署。启用隔离时必须为该用户已有运行配置一并分配端口，不能只隐藏节点却遗留不限量服务。原始启动/重启动作不能恢复共享或状态不明的旧配置，必须重新部署；关闭能力后重新开启只恢复管理入口，不擅自启动这类配置。

这是面板权限与固定监听端口计费隔离，不是独立进程、文件系统或 OS 沙箱。内核进程仍共用主机运行身份与资源；不能作为运行恶意配置、宿主机本地用户或 root 的安全边界。不启用隔离的用户仍按原能力访问主机，应只授予可信操作者。Sub-Store 共用后端地址，隔离用户可以管理自己的同步组，但不能修改后端连接设置；远端组名必须全局唯一。完整操作见 [用户与 Agent 分配](agent-sharing.md)。
