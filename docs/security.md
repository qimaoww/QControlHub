...
节点所有者可以在添加节点时勾选、或之后在节点设置里切换“不让管理员查看管理”。开启后该节点对管理员和兼容令牌在节点列表、配置、任务、日志、指标、流量、审计和安装凭据中一律不可见，直接访问返回 404；管理员和兼容令牌仍可在“其他用户节点”目录中只读看到名称、归属账号、在线状态、内核与端口，并保留删除该节点的能力，但部署、改配置、启停、共享和修改隐藏状态仍被拒绝。后台采集、计费、通知和 Agent 连接不受影响，所有者随时可恢复可见。
