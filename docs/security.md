...
借用者不能执行 `read-config`、`read-managed-config`、`import-existing` 或读取宿主配置快照。自有节点在没有他人配置或状态不明的部署时可显式读取/导入；读取任务在执行和完成时再次核验当前部署归属，节点所有者也不能借排队任务取得共享者的完整配置。普通用户的手动配置页默认编辑个人工作区，具备 `agent-config.write` 即可保存，无需执行任务权限；部署及校验另需 `tasks.execute`。模板应用同时需要 `templates.write` 和 `agent-config.write`。工作区内附带的 Agent 指标同样要求 `metrics.read`。
