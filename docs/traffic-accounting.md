...
配置页不再提供独立的“生成入站与出口成套文件”按钮。Xray / sing-box 源码通过“保存并部署”确认当前编辑内容，保存一个数据库版本并提交常规部署任务，校验、生成源码包、备份并重启；失败沿用原有回滚流程，取消确认不写入。成套源码包需要声明 `config-files-paired-v1` 的 Agent，旧 Agent 的普通校验和部署入口保持不变。外部系统服务仍须通过“手动导入并迁移”单独确认。运行入口仍是固定路径的合并配置，不改用内核原生目录合并启动方式。
