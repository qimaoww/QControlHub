...
已完成迁移的 sing-box 配置会保留 `log` 对象中的级别、时间戳等策略，但任何文件形式的 `log.output` 都会归一化为 `stdout`；手工部署同样拒绝文件输出。systemd 通过 volatile journal、OpenRC 通过受限轮转文件采集并上报，Agent 节点只保留面板容量策略约束的传输缓存，历史日志只在面板数据库保存。服务切换前读取器会先建立尾部游标，避免把导入启动日志误作旧历史跳过。旧 Agent 已经完成的迁移会在升级启动时事务性补做归一化：活动托管服务重启失败则恢复旧配置，成功后清空受保护 `/var/log` 或 sing-box 状态目录中的退役日志。支持 `core-log-status-v1` 的在线 Agent 会把等待、可用或采集失败状态报告给面板；旧 Agent 的日志状态明确显示为不可验证，离线 Agent 的持久化状态也不会冒充当前来源正常。
