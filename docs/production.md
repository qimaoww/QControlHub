...
填入预设只更新草稿：替换同名参数的草稿值，保留其他已勾选项，仍需检查后点击“保存并应用”并确认提交。节点缺少任何一项上报参数，或控制面字段规则不接受预设值时，整套预设都不会填入。提交时再次检查上报参数；确认期间所选参数失报或当前值变化会中止提交，保留草稿供重新核对，失报的已选项可单独取消勾选。预设不会重置未列出的系统值或已有托管项；32 MiB 是缓冲区上限，应结合节点内存和并发连接量评估，预设名称不代表性能保证。
...
控制台生成的一键安装命令使用 POSIX `sh`。在 Alpine Linux 上，安装器会通过 `apk` 补齐 `ca-certificates`、`coreutils`、`curl`、`iproute2`、`libcap`、`nftables` 与 `openrc`，自动写入 `/etc/init.d/qagent*`、`/etc/conf.d/qagent` 并加入 default runlevel；没有 `sudo` 的主机应先切换为 root。普通 systemd 主机继续使用原来的受限 unit。

OpenRC 模式设置 `QCH_SERVICE_MANAGER=openrc`，默认核心服务名去掉 `.service` 后缀。四个专用核心仍以 `qcontrolhub-core` 用户运行，并由 OpenRC 的 `supervise-daemon` 提供重启和 `CAP_NET_BIND_SERVICE`。QAgent 保留配置原子替换需要的 `CAP_CHOWN`，以及仅在派生真实内核校验进程时切换到 `qcontrolhub-core` 所需的 `CAP_SETUID`、`CAP_SETGID`；但只在当前进程的 Linux capability bounding set 确实包含 `CAP_NET_ADMIN` 时才向 `supervise-daemon` 申请流量管理能力。Podman 等受限容器缺少该能力时 Agent 仍可上线并管理核心，面板会把 nftables 流量执行标记为不可用。一键安装与升级会检测 `nft`，缺失时通过 apt、apk、dnf、yum 或 zypper 安装 `nftables`；同样会检测 `ip`，缺失时安装 `iproute2`（dnf/yum 上为 `iproute`）。没有受支持包管理器或安装失败时会在修改 Agent 服务前停止并给出明确错误。`ip` 不是可选的排障工具：没有 `with_v2ray_api` 的 sing-box 官方构建依赖独立 `routing_mark`，Agent 必须在部署前用 `ip rule show` 确认不存在会劫持这些标记的既有 fwmark 策略路由，缺少 `iproute2` 时相关校验与部署会明确失败而不是静默降级。核心面板实时日志流与 systemd 节点一致：`supervise-daemon` 将托管核心输出写入 `/var/log/qagent/<服务名>.log`，Agent 从文件尾持续跟踪新行并经由同一 `core-logs-v1` 通道上报；这些文件只是传输缓存，当前文件与面板所选数量的旧文件会共同平分单节点容量上限，超限旧文件不会被整份复制。systemd 节点优先使用 `Storage=volatile` 的 `qagent-cores` journal namespace；若运行环境不支持独立 namespace，则改用 `/run/qagent-core-logs/*.log` 易失文件继续向面板上报，不写入主机持久 journal。该目录由 qagent 的 systemd `RuntimeDirectory` 提供，因此在严格只读沙箱内仍能执行限额，且重启机器时自动消失。journal 与 `/run` 回退文件各自只分配单节点预算的一半，即使节点上暂时同时运行两种启动状态，传输缓存总量也不会突破面板上限。Agent 重启且尚未收到面板策略时会先使用最小的 1 MiB、零旧文件策略，不会临时恢复成较大的产品默认值；已收到的策略在后续核心安装中也不会被默认值覆盖。日志后端不可用不会阻塞核心服务启动。OpenRC 也没有 `ProtectSystem=strict` 等 systemd 沙箱，Agent 仍必须作为高权限 root 服务管理。
...
所有托管内核都只能向服务控制台输出日志：Mihomo 本身只提供控制台日志；Xray 的 `log.access`/`log.error` 文件路径、sing-box 的文件形式 `log.output`，以及 Shadowsocks Rust 的外部 `log.config_path`、file writer 和 syslog writer，都会在导入时归一化为控制台输出，手工部署也会拒绝这些持久化出口。systemd 通过 volatile journal、OpenRC 通过受限轮转文件采集并上报，Agent 节点只保留面板容量策略约束的传输缓存，历史日志只在面板数据库保存。服务切换前读取器会先建立尾部游标，避免把导入启动日志误作旧历史跳过。升级启动时会事务性检查旧 Agent 留下的普通面板部署和已完成迁移：活动托管服务重启失败则恢复旧配置，成功后清空位于受保护 `/var/log` 或对应内核状态目录内、能够从旧配置精确识别的退役日志。支持 `core-log-status-v1` 的在线 Agent 会把等待、可用或采集失败状态报告给面板；旧 Agent 的日志状态明确显示为不可验证，离线 Agent 的持久化状态也不会冒充当前来源正常。
...
在线升级会核对候选程序的大小、SHA-256 与 ELF 架构，并在原进程的权限限制下运行 `upgrade-preflight`；准备失败保持旧程序不变，替换前保存 `.previous-<sha256>` 备份，结果写入失败或 `exec` 失败会回滚到校验通过的备份，确认丢失后重新建立连接也会完成待处理的重启。新进程启动后只保留最新一份摘要与内容一致的回退副本，并清理历次升级累积的更旧副本；不符合严格名称、所有权、权限与摘要校验的相似文件不会被删除。没有预检协议的旧程序不会被在线降级执行，需要人工回退。OU-SB 证书新导入使用 `/etc/qagent/sing-box/ou-sb/<摘要>/`，SS Rust ACL 使用对应的 `/etc/qagent/shadowsocks-rust/` 子目录，兼容旧 Agent 的只读沙箱。
...
首次运行一键安装脚本时可逐项选择 Mihomo、Xray、sing-box、Shadowsocks Rust。非交互安装可预设 `QCH_DEFAULT_AGENT_ENGINES=xray,sing-box`；`none` 表示全部默认关闭，未指定则默认全部启用。内置和外部 PostgreSQL 部署均支持。

该环境变量只初始化数据库中的 `default_agent_engines`，不会在每次启动时覆盖面板保存的值。安装完成后，在系统设置的“全局内核默认能力”修改新 Agent 的默认值；已有节点不受修改影响。数据库升级保留现有节点能力，并记录 Agent 原始支持的内核集合。

在“节点设置 → 内核 → 节点内核能力”可以独立开关，允许开启全局默认关闭的内核，也允许全部关闭用于纯监控。关闭会下发停止服务任务；重新开启会恢复管理能力并启动已安装内核。启停成功后才更新开关，失败或取消会保留原能力，页面显示失败状态，可再次切换或在任务页重试。离线节点保留排队任务，重新上线后执行。未安装内核只切换管理能力，不自动安装。

切换期间阻止对应内核的新任务与配置写入，也不接受反向切换，避免刚启动又被旧停止任务覆盖；其他内核不受影响。有待处理或执行中的内核任务时，应先完成或取消任务再切换。该开关不会删除配置、日志、流量历史或卸载内核，已有流量规则仍保留。全局默认开关仍只影响新节点注册，不向已有节点批量下发启停任务。

节点重装使用原可复用凭据时保留其能力选择，并以新 Agent 声明的支持范围为限。常规安装的 Agent 声明全部四种内核；手动限制过 `QCH_AGENT_ENGINES` 的旧节点，须调整该环境并重新注册后才能开启原先未声明支持的内核。
