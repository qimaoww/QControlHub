...
在“节点设置 → 内核 → 节点内核能力”可以独立开关，允许开启全局默认关闭的内核，也允许全部关闭用于纯监控。它控制面板中新任务和配置写入的资格，不是服务启停开关：关闭不会自动停止服务、删除配置/日志/流量历史或卸载内核，已有流量规则仍保留；如需停止代理，请先停止对应服务。有待处理或执行中的内核任务时，应先完成或取消任务再关闭能力。
