...
已完成迁移的 sing-box 配置会保留原始 `log` 对象。未设置 `log.output`，或显式设置为 `stdout`/`stderr` 时，systemd 继续采集 journal，OpenRC 继续采集托管服务输出文件；服务切换前 Agent 会先确认对应读取器已经就绪，避免遗漏导入启动日志。设置文件形式的 `log.output` 时，路径相对 `/var/lib/qcontrolhub-sing-box` 解析，也可直接使用该目录内的绝对路径。为避免 Agent 被配置成任意文件读取器，目录外路径会在迁移时拒绝，采集时还会重新验证迁移 ownership、配置摘要、当前 engine 与服务完成态、逐级 owner/mode、无符号链接和普通文件身份；文件尚未创建时等待，创建、截断或 rename 轮转后自动续传。导入切换使用受保护 inode/offset 窗口，只补采本次迁移期间生成的内容，不读取更早的历史文件。Agent 不会改写或轮转该用户配置的输出文件。支持 `core-log-status-v1` 的 Agent 会把等待、可用或采集失败状态报告给面板；旧 Agent 的日志状态明确显示为不可验证，而不会冒充当前来源正常。
