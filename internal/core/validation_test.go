...
func TestActionWhitelistIncludesCurrentConfigurationRead(t *testing.T) {
	t.Parallel()
	for _, action := range []Action{ActionValidate, ActionDeploy, ActionStart, ActionStop, ActionRestart, ActionStatus, ActionInstall, ActionReadConfig, ActionReadManagedConfig, ActionImportExisting} {
		if !action.Valid() {
