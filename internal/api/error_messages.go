...
var errorMessages = map[string]string{
	...
	"stop and settle shared core instances before deleting the account":                                       "请先停止共享节点上的独立内核并完成流量结算，再删除账号。",
	...
	"invalid, reserved or duplicate shared port":                                                              "端口无效或重复；0 仅可单独使用，10085、10086 不可分配。",
	...
	"stop the legacy shared core before deploying its private instance":                                      "旧版共享内核仍在运行，请管理员先停止，再部署独立实例。",
	"shared Shadowsocks Rust instances do not support host-wide mainland access policies":                   "共享 Shadowsocks Rust 实例不支持影响整台节点的大陆访问限制，请先关闭该限制。",
...
var errorMessagePatterns = []struct {
	...
	{regexp.MustCompile(`^stop the uncertain (.+) core before reserving shared ports$`), "${1} 的部署状态不明，请先停止该内核，再分配端口。"},
