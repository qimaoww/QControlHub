...
func TestMain(tests *testing.M) {
	...
	cancel()
	if err != nil {
		fmt.Fprintf(os.Stderr, "prepare isolated store test database: %v\n", err)
		os.Exit(1)
	}
	if err := os.Setenv("QCH_TEST_DATABASE_URL", schema.URL); err != nil {
		...
		cleanupCancel()
		fmt.Fprintf(os.Stderr, "configure isolated store test database: %v\n", err)
		os.Exit(1)
	}
	code := tests.Run()
	cleanupContext, cleanupCancel := context.WithTimeout(context.Background(), 15*time.Second)
	if err := schema.Close(cleanupContext); err != nil {
		fmt.Fprintf(os.Stderr, "clean isolated store test database: %v\n", err)
		code = 1
	}
	cleanupCancel()
	os.Exit(code)
