...
func TestOpenWithMigrationsDisabledNeverChangesSchema(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("create isolated no-migration schema: %v", err)
	}
	defer func() {
		cleanupCtx, cleanupCancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cleanupCancel()
		if err := schema.Close(cleanupCtx); err != nil {
			t.Errorf("drop isolated no-migration schema: %v", err)
		}
	}()

	setup, err := pgx.Connect(ctx, schema.URL)
	if err != nil {
		t.Fatalf("connect to isolated no-migration schema: %v", err)
	}
	defer setup.Close(context.Background())

	_, err = OpenWithConfigKeyringOptions(ctx, schema.URL, true, "", nil, OpenOptions{DisableMigrations: true})
	if err == nil || !strings.Contains(err.Error(), "schema is not initialized") {
		t.Fatalf("open empty schema with migrations disabled error = %v", err)
	}
	var ledgerExists bool
	if err := setup.QueryRow(ctx, `SELECT to_regclass('qcontrolhub_schema_migrations') IS NOT NULL`).Scan(&ledgerExists); err != nil {
		t.Fatalf("inspect untouched schema: %v", err)
	}
	if ledgerExists {
		t.Fatal("migration-disabled open initialized the migration ledger")
	}
	...
	`); err != nil {
		t.Fatalf("create stale migration ledger: %v", err)
	}
	if _, err := setup.Exec(ctx, `INSERT INTO qcontrolhub_schema_migrations (version) VALUES ($1)`, currentSchemaVersion-1); err != nil {
		t.Fatalf("record stale schema version: %v", err)
	}
	_, err = OpenWithConfigKeyringOptions(ctx, schema.URL, true, "", nil, OpenOptions{DisableMigrations: true})
	if err == nil || !strings.Contains(err.Error(), "migrations are disabled") {
		t.Fatalf("open stale schema with migrations disabled error = %v", err)
	}
	var version int
	if err := setup.QueryRow(ctx, `SELECT max(version) FROM qcontrolhub_schema_migrations`).Scan(&version); err != nil {
		t.Fatalf("read unchanged stale schema version: %v", err)
	}
	if version != currentSchemaVersion-1 {
		t.Fatalf("migration-disabled open changed schema version to %d", version)
	}

	if _, err := setup.Exec(ctx, `UPDATE qcontrolhub_schema_migrations SET version=$1`, currentSchemaVersion); err != nil {
		t.Fatalf("mark fixture schema current: %v", err)
	}
	dataStore, err := OpenWithConfigKeyringOptions(ctx, schema.URL, true, "", nil, OpenOptions{DisableMigrations: true})
	if err != nil {
		t.Fatalf("open matching schema with migrations disabled: %v", err)
	}
	dataStore.Close()
}
