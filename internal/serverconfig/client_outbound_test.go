		...
		for _, protocol := range Protocols(source) {
			for _, target := range []core.Engine{core.EngineXray, core.EngineSingBox} {
				t.Run(string(source)+"/"+protocol.Key+"/"+string(target), func(t *testing.T) {
					...
					unsupported := protocol.UsesEndpoint || input.Protocol == ProtocolWireGuard || isSnellProtocol(input.Protocol) || input.Protocol == ProtocolSudoku || input.Protocol == ProtocolMieru || input.Protocol == ProtocolPortForward ||
