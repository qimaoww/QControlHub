...
func TestGenerateServerConfigurations(t *testing.T) {
	...
	tests := []struct {
		...
		{"xray vless", core.EngineXray, Input{Protocol: ProtocolVLESS, Tag: "vless-in", Listen: "0.0.0.0", Port: 443, Username: "user@example.com", Credential: uuid, Flow: "xtls-rprx-vision", Transport: "raw", TLSEnabled: true, CertificatePath: "/etc/tls/server.crt", PrivateKeyPath: "/etc/tls/server.key"}, []string{`"protocol": "vless"`, `"streamSettings"`, `"certificateFile"`}},
