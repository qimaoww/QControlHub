...
grep -Fq -- "--depth 1 --branch main --single-branch https://github.com/qimaoww/qcontrolhub.git $install_dir" "$QCH_BOOTSTRAP_TEST_LOG"
...
grep -Fq -- "fetch --prune origin main" "$QCH_BOOTSTRAP_TEST_LOG"
grep -Fq -- "merge --ff-only FETCH_HEAD" "$QCH_BOOTSTRAP_TEST_LOG"
