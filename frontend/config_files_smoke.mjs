...
assert.deepEqual(named.map(f=>f.path), ["common.json","inbounds/VLESS-REALITY-8443.json","inbounds/香港入口.json","inbounds/香港入口-2.json","inbounds/_a_b.json","inbounds/inbound-5.json",`inbounds/${"a".repeat(48)}.json`,"outbounds/direct.json"]);
...
for (const engine of ["xray","sing-box"]) {
  ...
  assert.deepEqual(files.map(f=>f.path),["common.json","inbounds/a.json","inbounds/b.json","outbounds/direct.json"]);
