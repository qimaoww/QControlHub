export function createAccessControlView({ state, esc, engineName, shell }, { editable }) {
  const card = (entry) => `<article class="access-control-card" data-refresh-key="access-control-${esc(entry.agent_id)}-${esc(entry.engine)}-${esc(entry.tag)}">
          ...
          <form data-access-control-form data-agent-status="${esc(entry.agent_status)}">
            ...
            <label class="access-control-option"><input type="checkbox" name="block_mainland_source" ${entry.block_mainland_source ? "checked" : ""} ${editable() && (entry.kind !== "wireguard" || entry.block_mainland_source) ? "" : "disabled"}><span><b>禁止大陆来源连接</b><small>${entry.kind === "wireguard" ? "WireGuard 路由只能识别隧道内源地址，公网来源限制须在节点防火墙配置；已有此规则时请关闭。" : entry.engine === "ss-rust" ? "使用 nftables 按入站端口拒绝当前 CN IP 源的 IPv4 来源，规则独立于 ssserver 配置。" : "来源 IPv4 / IPv6 命中大陆 CIDR 时，仅拒绝连接这个入站端口。"}</small></span><em>${entry.kind === "wireguard" ? "不支持" : entry.block_mainland_source ? "已启用" : "未启用"}</em></label>
