...
export function installAgents(ctx) {
  ...
  const komariCardMarkup = (agent) => {
    const uuid = komariUUIDFor(agent);
    if (!uuid) return "";
    return `<section class="node-card-komari" data-komari-link="${esc(agent.id)}"><header><span>Komari 每月流量</span><code>${esc(short(uuid))}</code></header><strong data-komari-traffic>读取中…</strong><small data-komari-cycle>计费周期读取中</small></section>`;
  };
  const updateKomariDisplay = (root, link, error = "") => {
    ...
    if (error || !link?.server) {
      if (traffic) traffic.textContent = error || "未读取到 Komari 设置";
      if (cycle) cycle.textContent = "请检查地址、API Key 和服务器 UUID";
    ...
    const limitType = hasEffectiveLimit && server.effective_traffic_type ? server.effective_traffic_type : server.traffic_limit_type || "sum";
    if (traffic) traffic.textContent = limit > 0 ? `${bytes(limit)} / 周期（${limitType}）` : "不限流量";
    if (cycle) cycle.textContent = server.billing_cycle > 0 ? `计费周期 ${server.billing_cycle} 天${server.traffic_reset_day > 0 ? ` · 每月 ${server.traffic_reset_day} 日重置` : ""}` : "未设置计费周期";
