...
export function createDashboardView(ctx, panelMetrics) {
  ...
  const trend = summarizeDashboardTraffic(dailyTraffic, trafficMonth);
  ...
  const trafficHistory = can("traffic.read") ? `<section class="traffic-history dashboard-traffic-history qch-swap-panel" id="traffic-usage" data-refresh-key="dashboard-traffic-${esc(trafficMonth)}">
    ...
    <div class="dashboard-traffic-summary"><div class="dashboard-traffic-total"><span>${trend.isCurrentMonth ? "本月累计" : "当月累计"}</span><strong>${bytes(usedTraffic)}</strong></div><dl><div><dt><i class="received" aria-hidden="true"></i>接收</dt><dd>${bytes(receivedTraffic)}</dd></div><div><dt><i class="sent" aria-hidden="true"></i>发送</dt><dd>${bytes(sentTraffic)}</dd></div></dl></div>
    <dl class="dashboard-traffic-trends" aria-label="流量趋势摘要">
      <div data-dashboard-trend="today"><dt>今日用量</dt><dd><strong>${trend.todayBytes === null ? "—" : bytes(trend.todayBytes)}</strong><small>${trend.isCurrentMonth ? "UTC 今日累计" : "非当前月份"}</small></dd></div>
      <div data-dashboard-trend="average"><dt>日均用量</dt><dd><strong>${trend.averageBytes === null ? "—" : bytes(trend.averageBytes)}</strong><small>${trend.elapsedDays ? `按 ${trend.elapsedDays} 天${trend.isCurrentMonth ? "（含今日）" : "计算"}` : "月份尚未开始"}</small></dd></div>
      <div data-dashboard-trend="peak"><dt>${trend.isCurrentMonth ? "本月" : "当月"}峰值日</dt><dd><strong>${trend.peakDay ? `${Number(trend.peakDay.slice(5, 7))}月${Number(trend.peakDay.slice(8))}日` : "—"}</strong><small>${trend.peakDay ? `${bytes(trend.peakBytes)} · 单日用量` : "暂无流量"}</small></dd></div>
    </dl>
