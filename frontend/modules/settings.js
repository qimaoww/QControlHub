  ...
  async function settings() {
    ...
    shell(`<div class="settings-workspace settings-overview">
      <form class="settings-form" id="settings-form">
        ...
        ${section("settings-notify", "05", "事件通知", "Webhook 地址由控制面调用，签名密钥继续通过部署环境配置。", `<div class="settings-grid one-column">${field("webhook_url", "Webhook 地址", `<input name="webhook_url" type="url" value="${esc(item.webhook_url || "")}" maxlength="500" placeholder="https://example.com/hooks/qcontrolhub" ${disabled}>`)}</div><div class="settings-toggle-list">
