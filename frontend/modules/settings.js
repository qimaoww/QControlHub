...
async function settings() {
  const [item, auditLogs, users] = await Promise.all([
    api("/settings"),
    api("/audit?limit=100"),
    can("admin") ? api("/users") : Promise.resolve([]),
  ]);
  ...
  const userRows = users
    .map(
      (user) =>
        `<form class="panel-user-row" data-user-form="${esc(user.id)}"><div><strong>${esc(user.username)}</strong><small>${esc(user.display_name || "未设置姓名")} · ${user.disabled ? "已停用" : "正常"}</small></div><select name="role" aria-label="${esc(user.username)} 角色">${["admin", "operator", "readonly"].map((role) => `<option value="${role}" ${user.role === role ? "selected" : ""}>${roleName[role]}</option>`).join("")}</select><input name="display_name" value="${esc(user.display_name || "")}" maxlength="100" placeholder="显示名称" aria-label="${esc(user.username)} 显示名称"><input name="password" type="password" minlength="12" maxlength="72" autocomplete="new-password" placeholder="留空不修改" aria-label="${esc(user.username)} 新密码"><label class="user-disabled-toggle"><input name="disabled" type="checkbox" ${user.disabled ? "checked" : ""} ${state.session.user_id === user.id ? "disabled" : ""}>停用</label><button class="button small" type="submit">保存</button>${state.session.user_id === user.id ? "" : `<button class="button small danger-button" type="button" data-disable-user="${esc(user.id)}" ${user.disabled ? "disabled" : ""}>停用</button>`}</form>`,
    )
    .join("");
  const userSection = can("admin")
    ? `<section class="settings-section" id="users"><header><span class="settings-section-number">05</span><h3>用户管理</h3></header><p class="settings-hint">为团队成员创建独立账号，权限变更会立即使其现有会话失效。至少保留一个启用中的管理员。</p><form class="user-create-form" id="user-create-form"><input name="username" required maxlength="64" pattern="[A-Za-z0-9._-]+" placeholder="用户名"><input name="display_name" maxlength="100" placeholder="显示名称"><select name="role" aria-label="新用户角色"><option value="operator">操作员</option><option value="readonly">只读</option><option value="admin">管理员</option></select><input name="password" type="password" required minlength="12" maxlength="72" autocomplete="new-password" placeholder="初始密码（至少 12 字节）"><button class="button primary" type="submit">添加用户</button></form><div class="panel-user-list">${userRows || '<div class="empty compact"><strong>还没有个人用户</strong></div>'}</div></section>`
    : "";
  shell(
  ...
  document.querySelectorAll("[data-user-form]").forEach((formElement) => {
