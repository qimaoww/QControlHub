...
async function liveConfig() {
  ...
  const engineBar = `<nav class="live-engine-bar" aria-label="选择内核">${installedEngines.map(item => {
    ...
    return `<button type="button" class="live-engine-tab ${active ? "active" : ""}" data-live-engine="${esc(item)}" aria-pressed="${active}" aria-label="${esc(engineName(item))} · ${info.installed ? "已安装" : "待导入"}" title="${info.installed ? "已安装" : "待导入"}" ${active ? 'aria-current="true"' : ""}><span class="engine-badge ${esc(item)}">${esc(engineName(item))}</span><i class="live-engine-state ${info.installed ? "installed" : ""}" aria-hidden="true"></i></button>`;
  }).join("")}</nav>`;
  shell(
  ...
  const configFiles = bindConfigFiles(document.querySelector("#live-config-form"), engine, notify, agent.name);
  bindCodeEditors();
