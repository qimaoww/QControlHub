...
export function createIPQualityReportView({ esc }) {
  ...
  return (report) => {
    ...
    return `<section class="ip-quality-report">
      <h4>${String(report.Head?.IP || "").includes(":") ? "IPv6" : "IPv4"} · ${value(report.Head?.IP)}</h4>
      ...
      <h5>DNS 黑名单（不是 DNS 泄漏检测）</h5>${pairs([["数据库总数", blacklist.Total], ["干净", blacklist.Clean], ["标记", blacklist.Marked], ["列入黑名单", blacklist.Blacklisted]])}
