...
export function createIPQualityReportView({ esc }) {
  ...
  // Risk factors stay open: the per-provider values are the evidence the risk
  // scores point at, and folding each factor into a disclosure made the whole
  // section read as a list of empty rows.
  const factors = (factorData) => ipQualityEntries(factorData).map(([factor, providers]) => `<section class="ip-quality-factor"><strong>${esc(factorLabels[factor] || factor)}</strong>${pairs(ipQualityEntries(providers))}</section>`).join("") || "<p>未知</p>";
  return (report) => {
    ...
    return `<section class="ip-quality-report">
      ...
      <section class="ip-quality-report-group"><h5>风险因子</h5><div class="ip-quality-factors">${factors(report.Factor)}
      </div></section>
