...
export function createIPQualityReportView({ esc }) {
  ...
  return (report) => {
    ...
    return `<section class="ip-quality-report">
      ...
      <section class="ip-quality-report-group"><h5>风险评分（各数据源独立）</h5>${pairs(ipQualityEntries(report.Score))}</section>
      ...
      <section class="ip-quality-report-group"><h5>DNS 黑名单（不是 DNS 泄漏检测）</h5>${ipv6 ? "<p>未检测：当前上游仅查询 IPv4 DNS 黑名单。原始 JSON 中的 IPv6 黑名单数字可能沿用 IPv4 计数，不能作为本地址的检测结果。</p>" : pairs([["数据库总数", blacklist.Total], ["干净", blacklist.Clean], ["标记", blacklist.Marked], ["列入黑名单", blacklist.Blacklisted]])}
      </section></div>
