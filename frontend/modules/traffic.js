...
// Agent counters are named from the listener's perspective: received bytes
// enter the monitored port (client upload), while sent bytes leave it (client
// download). Keep that wire/API naming stable and translate it once for UI.
export function portTrafficDirections(traffic = {}) {
  return {
    downloadBytes: Number(traffic.sent_bytes || 0),
    uploadBytes: Number(traffic.received_bytes || 0),
    downloadBPS: Number(traffic.send_bps || 0),
    uploadBPS: Number(traffic.receive_bps || 0),
  };
}
...
export function installTraffic(ctx) {
  ...
  const renderTraffic = (agents, policies, endpoints, resetCreate = false) => {
    ...
    const cards = filteredItems.map((item) => {
      ...
      const directions = portTrafficDirections(policy);
      const downloadBPS = trafficRateForDisplay(directions.downloadBPS, policy.last_reported_at, agent?.status);
      const uploadBPS = trafficRateForDisplay(directions.uploadBPS, policy.last_reported_at, agent?.status);
      ...
      return `<article class="traffic-policy-card ${policy.blocked ? "is-blocked" : ""}" id="traffic-${esc(policy.id)}" data-refresh-key="traffic-policy-${esc(policy.id)}" data-traffic-agent-card="${esc(policy.agent_id)}" data-traffic-card-key="${esc(trafficCardIdentity(item))}">
        ...
        <section class="traffic-card-transfer"><div><i class="download" aria-hidden="true">↓</i><span><small>下载流量</small><strong>${bytes(directions.downloadBytes)}</strong></span><em>${rate(downloadBPS)}</em></div><div><i class="upload" aria-hidden="true">↑</i><span><small>上传流量</small><strong>${bytes(directions.uploadBytes)}</strong></span><em>${rate(uploadBPS)}</em></div></section>
