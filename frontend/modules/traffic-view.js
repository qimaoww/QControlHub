...
export function createTrafficView(ctx, { filters, storage, savedTrafficCardOrder, policyFields, selectOptions }) {
  ...
  return (agents, policies, endpoints) => {
    ...
    for (const agent of agents) {
      ...
      if (seenNodeNames.has(name)) duplicateNodeNames.add(name);
    ...
    const endpointChoices = items.map((item) => {
      const value = item.policy || item.endpoint;
      const nodeName = agentByID.get(value.agent_id)?.name || value.agent_id;
      const nodeLabel = duplicateNodeNames.has(nodeName) ? `${nodeName} (${value.agent_id})` : nodeName;
      return {
        value: item.key,
        label: `${nodeLabel} · ${value.name || "未命名端口"} · ${engineName(value.engine)} :${value.port}`,
      };
    }).sort((a, b) => a.label.localeCompare(b.label));
    const selectedScope = currentFilters.agent_id
      ? agentByID.get(currentFilters.agent_id)?.name || "所选节点"
      : "全部节点";
    const toolbar = `<section class="traffic-control-panel"><section class="traffic-total" aria-label="所选端口流量概览"><span data-traffic-refresh-label>${esc(selectedScope)} · 所选端口累计</span><div><strong>${bytes(totalUsed)}</strong>${quotaPolicies.length ? `<small>/ ${bytes(totalLimit)}</small>` : ""}</div>${quotaPolicies.length ? `<progress aria-label="流量占配额比例" max="100" value="${percent(totalUsed, totalLimit)}"></progress>` : `<span class="traffic-total-metering">${filteredPolicies.length ? "全部端口正在持续统计" : "等待 Agent 建立监控"}</span>`}<em>${filteredPolicies.length} 个监控端口 · ${quotaPolicies.length} 个配额</em></section><div class="traffic-filters" aria-label="流量筛选">
    ...
    const createDialog = can("traffic.manage")
      ...
      : "";
    const renderPortIdentity = (value, agent) => {
      const nodeName = agent?.name || value.agent_id;
      return `<div class="traffic-port-number"><span>监听端口</span><strong>:${esc(value.port)}</strong></div>
        <div class="traffic-port-context"><div class="traffic-port-node"><span>节点</span><strong title="节点 ID：${esc(value.agent_id)}">${esc(nodeName)}</strong>${duplicateNodeNames.has(nodeName) ? `<small>节点 ID ${esc(value.agent_id)}</small>` : ""}</div><div class="traffic-port-core"><span>内核</span><span class="engine-badge ${esc(value.engine)}">${esc(engineName(value.engine))}</span></div></div>
        <div class="traffic-port-config"><span>配置</span><strong title="${esc(value.name || "未命名端口")}">${esc(value.name || "未命名端口")}</strong><span class="traffic-protocol-label">${esc(protocolName(value.protocol))}</span></div>`;
    };
    const cards = filteredItems.map((item) => {
      if (item.kind === "endpoint") {
        ...
        return `<article class="traffic-policy-card traffic-configured-port" data-refresh-key="traffic-endpoint-${esc(item.key)}" data-traffic-agent-card="${esc(endpoint.agent_id)}" data-traffic-card-key="${esc(trafficCardIdentity(item))}">
          <header class="traffic-port-header">${renderPortIdentity(endpoint, agent)}<span class="traffic-card-controls"><span class="traffic-policy-status warn"><i></i>等待同步</span><span class="node-card-grip traffic-card-grip" title="拖动调整顺序" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M9 6h.01M9 12h.01M9 18h.01M15 6h.01M15 12h.01M15 18h.01"/></svg></span></span></header>
