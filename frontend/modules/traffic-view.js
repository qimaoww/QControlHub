...
export function createTrafficView(ctx, { filters, storage, savedTrafficCardOrder, policyFields, selectOptions }) {
  ...
  return (agents, policies, endpoints) => {
    ...
    const selectedScope = currentFilters.agent_id
      ...
      : "全部节点";
    const toolbar = `<section class="traffic-control-panel"><header class="traffic-filter-head"><div><h2>流量筛选</h2><span data-traffic-refresh-label>${esc(selectedScope)}</span></div><button class="button small" type="button" data-traffic-filter-reset ${Object.values(currentFilters).some(Boolean) ? "" : "disabled"}>清除筛选</button></header><div class="traffic-filter-content"><div class="traffic-filters" aria-label="流量筛选">
    ...
    </div><section class="traffic-total" aria-label="流量概览"><span>所选端口累计</span><div><strong>${bytes(totalUsed)}</strong>${quotaPolicies.length ? `<small>/ ${bytes(totalLimit)}</small>` : ""}</div>${quotaPolicies.length ? `<progress aria-label="累计流量占配额合计比例" max="100" value="${percent(totalUsed, totalLimit)}"></progress>` : `<span class="traffic-total-metering">${filteredPolicies.length ? "全部端口正在持续统计" : "等待 Agent 建立监控"}</span>`}<em>${filteredPolicies.length} 个监控端口 · ${quotaPolicies.length} 个配额</em></section></div></section>`;
    ...
    const cards = filteredItems.map((item) => {
      if (item.kind === "endpoint") {
        ...
        return `<article class="traffic-policy-card traffic-configured-port" data-refresh-key="traffic-endpoint-${esc(item.key)}" data-traffic-agent-card="${esc(endpoint.agent_id)}" data-traffic-card-key="${esc(trafficCardIdentity(item))}">
          <header><div class="traffic-card-heading"><span class="engine-badge ${esc(endpoint.engine)}">${esc(engineName(endpoint.engine))}</span><strong title="${esc(endpoint.name || `端口 ${endpoint.port}`)}">${esc(endpoint.name || `端口 ${endpoint.port}`)}</strong></div><span class="traffic-card-controls"><span class="traffic-policy-status warn"><i></i>等待同步</span><span class="node-card-grip traffic-card-grip" title="拖动调整顺序" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M9 6h.01M9 12h.01M9 18h.01M15 6h.01M15 12h.01M15 18h.01"/></svg></span></span><div class="traffic-card-identity"><small><span class="engine-badge traffic-node-tag" title="${esc(agent?.name || endpoint.agent_id)}">${esc(agent?.name || endpoint.agent_id)}</span><code class="engine-badge traffic-port-tag">:${esc(endpoint.port)}</code><span class="engine-badge traffic-protocol-tag">${esc(protocolName(endpoint.protocol))}</span></small></div></header>
      ...
      return `<article class="traffic-policy-card ${policy.blocked ? "is-blocked" : ""}" id="traffic-${esc(policy.id)}" data-refresh-key="traffic-policy-${esc(policy.id)}" data-traffic-agent-card="${esc(policy.agent_id)}" data-traffic-card-key="${esc(trafficCardIdentity(item))}">
        <header><div class="traffic-card-heading"><span class="engine-badge ${esc(policy.engine)}">${esc(engineName(policy.engine))}</span><strong title="${esc(policy.name)}">${esc(policy.name)}</strong></div><span class="traffic-card-controls"><span class="traffic-policy-status ${tone}"><i></i>${esc(status)}</span><span class="node-card-grip traffic-card-grip" title="拖动调整顺序" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M9 6h.01M9 12h.01M9 18h.01M15 6h.01M15 12h.01M15 18h.01"/></svg></span></span><div class="traffic-card-identity"><small><span class="engine-badge traffic-node-tag" title="${esc(agent?.name || policy.agent_id)}">${esc(agent?.name || policy.agent_id)}</span><code class="engine-badge traffic-port-tag">:${esc(policy.port)}</code><span class="engine-badge traffic-protocol-tag">${esc(protocolName(policy.protocol))}</span></small></div></header>
        <div class="traffic-card-metrics"><section class="traffic-card-quota ${quotaEnabled ? "" : "is-monitor-only"}"><header><span>${quotaEnabled ? "当前周期用量" : "本月累计流量"}</span><b>${policy.shared_quota ? "共享额度" : quotaEnabled ? `${usedPercent.toFixed(1)}%` : "未设置配额"}</b></header><div><strong>${bytes(policy.used_bytes)}</strong>${quotaEnabled ? `<span>/ ${bytes(policy.limit_bytes)}</span>` : ""}</div>${quotaEnabled ? `<progress aria-label="当前周期配额使用比例" max="100" value="${usedPercent}"></progress>` : ""}<small>${esc(period)}${policy.shared_quota ? `<br>共享累计 ${(policy.shared_quota.used_bytes / gibibyte).toFixed(2)} / ${policy.shared_quota.limit_bytes ? `${(policy.shared_quota.limit_bytes / gibibyte).toFixed(2)} GiB` : "不限量"}` : ""}</small></section>
