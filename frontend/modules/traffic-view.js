    ...
    const cards = filteredItems.map((item) => {
      if (item.kind === "endpoint") {
        ...
        return `<article class="traffic-policy-card traffic-configured-port" data-refresh-key="traffic-endpoint-${esc(item.key)}" data-traffic-agent-card="${esc(endpoint.agent_id)}" data-traffic-card-key="${esc(trafficCardIdentity(item))}">
          <header><div class="traffic-card-identity"><span class="engine-badge ${esc(endpoint.engine)}">${esc(engineName(endpoint.engine))}</span><span><strong title="${esc(endpoint.name || `端口 ${endpoint.port}`)}">${esc(endpoint.name || `端口 ${endpoint.port}`)}</strong><small><span class="engine-badge traffic-node-tag" title="${esc(agent?.name || endpoint.agent_id)}">${esc(agent?.name || endpoint.agent_id)}</span><code class="engine-badge traffic-port-tag">:${esc(endpoint.port)}</code><span class="engine-badge traffic-protocol-tag">${esc(protocolName(endpoint.protocol))}</span></small></span></div><span class="traffic-card-controls"><span class="traffic-policy-status warn"><i></i>等待同步</span><span class="node-card-grip traffic-card-grip" title="拖动调整顺序" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M9 6h.01M9 12h.01M9 18h.01M15 6h.01M15 12h.01M15 18h.01"/></svg></span></span></header>
      ...
      return `<article class="traffic-policy-card ${policy.blocked ? "is-blocked" : ""}" id="traffic-${esc(policy.id)}" data-refresh-key="traffic-policy-${esc(policy.id)}" data-traffic-agent-card="${esc(policy.agent_id)}" data-traffic-card-key="${esc(trafficCardIdentity(item))}">
        <header><div class="traffic-card-identity"><span class="engine-badge ${esc(policy.engine)}">${esc(engineName(policy.engine))}</span><span><strong title="${esc(policy.name)}">${esc(policy.name)}</strong><small><span class="engine-badge traffic-node-tag" title="${esc(agent?.name || policy.agent_id)}">${esc(agent?.name || policy.agent_id)}</span><code class="engine-badge traffic-port-tag">:${esc(policy.port)}</code><span class="engine-badge traffic-protocol-tag">${esc(protocolName(policy.protocol))}</span></small></span></div><span class="traffic-card-controls"><span class="traffic-policy-status ${tone}"><i></i>${esc(status)}</span><span class="node-card-grip traffic-card-grip" title="拖动调整顺序" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M9 6h.01M9 12h.01M9 18h.01M15 6h.01M15 12h.01M15 18h.01"/></svg></span></span></header>
