// Display the stored SVG as an image, never as executable markup or an iframe.
  ...
  return (record) => {
    const figures = (record.archives || []).filter((archive) => [4, 6].includes(archive.family)).map((archive) => {
      ...
      return `<figure class="ip-quality-archive" data-refresh-key="archive-${esc(record.task_id)}-${archive.family}">
        <figcaption><strong>IPv${archive.family} 原始报告</strong><a class="button small" href="${source}?download=1" download>下载报告</a></figcaption>
        <a href="${source}" target="_blank" rel="noopener noreferrer" aria-label="放大查看 IPv${archive.family} 原始报告"><img src="${source}" alt="IPv${archive.family} IPQuality 检测报告" loading="lazy" referrerpolicy="no-referrer"></a>
        <small>已存档 · ${esc(date(archive.downloaded_at))} · 点击报告放大查看</small>
